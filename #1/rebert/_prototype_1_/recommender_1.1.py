...
# Add new constant: temperature & max completion token
OAI_MODEL_TEMPERATURE = 0
OAI_MODEL_MAX_TOKENS = 1000
...
#Modified
MOVIE_RECOMMENDER_PERSONA_PROMPT = '''You are a movie critic who wants to make sure that you make the best movie recommendations. Make sure that the movie you recommend satisfies the user across many movie attributes including actors, visuals, music, plot line, character development, dialog, mood, and many other movie attributes. Your recommendation should only focused on documentary. Your responses should always focus on making movie recommendations.'''
...
def new_chat_context():
    ...
    chat_context['max_tokens'] = OAI_MODEL_MAX_TOKENS
...
def make_chat_request(user_text="", chat_context=None, api_key=""):
   ...
   #   Add the response to our chat context
    ...
    chat_token_usage = resp_dict['usage']['total_tokens']
   
    return chat_context, chat_token_usage
...
def main():
    ...
    while len(user_text)>0 and (user_text.lower() != "quit"):
        ...
        chat_context, chat_token_usage = make_chat_request(user_text, chat_context, API_KEY)
        ...
        # Print Token Usage
        total_chat_token_usage += chat_token_usage
        print(chat_token_usage)
        print()
        print(total_chat_token_usage)
